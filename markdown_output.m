...
static NSMutableDictionary *merge(NSDictionary *into, NSDictionary *with) {
    ...
    // 'cascading' styles
    NSFont* inheritedFont = [into objectForKey:NSFontAttributeName];
    if (inheritedFont) {
        NSFont* elementFont = [with objectForKey:NSFontAttributeName];
        if (elementFont) {
            ...
            NSFont* newFont = [NSFont fontWithName:newFontName size:inheritedFont.pointSize];
    ...
    switch (elt->key) {
        ...
        case LINK:;
            ...
            if (url) {
                ...
                NSDictionary *attributesBroken = @{NSForegroundColorAttributeName: [NSColor redColor]}; // Make this attributes[BROKEN]
